...
function RFPList({ onBack }) {
  ...
  return (
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,color:BRAND.text,position:"relative",overflow:"hidden" }}>
      ...
      <header style={{ position:"sticky",top:0,zIndex:100,background:BRAND.glass,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:`1px solid ${BRAND.glassBorder}`,padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10,fontWeight:500,color:BRAND.textMuted,letterSpacing:"2.5px",textTransform:"uppercase" }}>Open RFPs — Brand Opportunities</div>
          </div>
        </div>
        <button onClick={onBack} style={{ ...ghostBtn,padding:"8px 16px",fontSize:12 }}>← Back</button>
      </header>

      <div style={{ position:"relative",zIndex:1,maxWidth:1200,margin:"0 auto",padding:"40px 24px 80px" }}>
        <div style={{ fontSize:10,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",color:BRAND.lightBlue,marginBottom:8 }}>Open Opportunities</div>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:12,marginBottom:24 }}>
          <div>
            ...
            <p style={{ fontSize:13,color:BRAND.textMuted,marginTop:4 }}>Brands and agencies looking for creators — apply directly.</p>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ background:BRAND.glass,backdropFilter:"blur(16px)",border:`1px solid ${BRAND.glassBorder}`,borderRadius:16,padding:"40px 24px",textAlign:"center" }}>
            <div style={{ fontSize:14,fontWeight:700,color:BRAND.navy,marginBottom:8 }}>No open RFPs right now</div>
            <div style={{ fontSize:12,color:BRAND.textMuted }}>Brands post campaigns here when they're looking for creators. Check back soon.</div>
          </div>
        ) : (
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16 }}>
            {filtered.map(c => {
              ...
              return (
                <div key={c.id} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
                  style={{ background:h?BRAND.glassHover:BRAND.glass,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:`1.5px solid ${h?BRAND.lightBlue:BRAND.glassBorder}`,borderRadius:16,padding:"22px 20px",transition:"all 0.2s",boxShadow:h?BRAND.shadowHover:BRAND.shadow,transform:h?"translateY(-3px)":"none" }}>
                  ...
                  <button onClick={()=>setApplying(c)} style={{ ...primaryBtn,width:"100%",padding:"11px 0",fontSize:13 }}>Apply Now →</button>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ textAlign:"center",padding:"32px 24px",fontSize:11,color:BRAND.textLight,letterSpacing:"1px" }}>
        CREATOR'S COMPASS · PRËMO INC. · CANGGU, BALI · {new Date().getFullYear()}
      </div>
    ...
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:24,position:"relative",overflow:"hidden" }}>
      ...
      <div style={{ width:"100%",maxWidth:580,position:"relative",zIndex:1 }}>
        <div style={{ textAlign:"center",marginBottom:40 }}>
          ...
          <p style={{ fontSize:14,color:BRAND.textMuted,maxWidth:420,margin:"0 auto 16px",lineHeight:1.6 }}>Tell us about your content — we'll find the best Bali businesses to partner with</p>
          {/* ── ADDED: RFP button ── */}
          ...
          {step===3 && (
            <>
              ...
              <div style={{ background:BRAND.skyBlue,borderRadius:12,padding:"14px 16px",marginTop:4 }}>
                ...
                <div style={{ fontSize:12,color:BRAND.textMuted,lineHeight:1.6 }}>The Compass finds matching Bali businesses, scores them by fit, and generates a personalized outreach message for each one.</div>
              </div>
            </>
          )}
...
async function generateOutreach(business, profile) {
  ...
  } catch(e) { return `Hi! I'm ${profile.name||"a creator"} — I make ${profile.niches[0]||"lifestyle"} content on ${profile.platforms[0]||"Instagram"}. I love what you do at ${business.name} and I think we could create something really special together. Would love to chat!`; }
}
    ...
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,color:BRAND.text,position:"relative",overflow:"hidden" }}>
      ...
      <header style={{ position:"sticky",top:0,zIndex:100,background:BRAND.glass,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:`1px solid ${BRAND.glassBorder}`,padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:12 }}>
          ...
          {/* ── ADDED: RFP button in results header ── */}
...
export default function App() {
  ...
  return (
    <>
      ...
      {stage==="onboard" && (
        <Onboarding
          onComplete={p=>{ setProfile(p); setStage("running"); }}
          onViewRFPs={()=>setStage("rfp")}
        />
      )}
      {stage==="running" && (
        <CompassRunning profile={profile} onResults={r=>{ setResults(r); setStage("results"); }}/>
      )}
      {stage==="results" && (
        <Results
          businesses={results}
          profile={profile}
          onNewSearch={()=>{ setStage("onboard"); setProfile(null); setResults([]); }}
          onViewRFPs={()=>setStage("rfp")}
        />
      )}
    </>

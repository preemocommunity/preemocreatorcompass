...
const STATUS_COLORS = {
  new:{bg:"#F3F0FF",text:"#5B3FC5"}, contacted:{bg:"#E8F4FD",text:"#1E3A5F"},
  in_negotiation:{bg:"#FFF3E0",text:"#B86E00"}, confirmed:{bg:"#E6F4EA",text:"#1B7A3D"},
  completed:{bg:"#E6F4EA",text:"#1B7A3D"}, passed:{bg:"#F5F5F5",text:"#888"},
};
 
const inp = { width:"100%",background:BRAND.ice,border:`1px solid ${BRAND.glassBorder}`,borderRadius:12,padding:"11px 14px",fontSize:13,color:BRAND.text,fontFamily:"'Montserrat', sans-serif",boxSizing:"border-box",outline:"none",marginBottom:12 };
const lbl = { fontSize:10,fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",color:BRAND.textLight,marginBottom:6,display:"block" };
...
function Tag({ children, active, onClick }) {
  return (
    <button onClick={onClick} style={{ padding:"6px 14px",borderRadius:10,border:active?"none":`1px solid ${BRAND.glassBorder}`,background:active?BRAND.navy:BRAND.glass,color:active?"#fff":BRAND.text,fontSize:12,fontWeight:600,fontFamily:"'Montserrat', sans-serif",cursor:"pointer",transition:"all 0.2s",marginBottom:4,backdropFilter:"blur(12px)",boxShadow:active?"0 4px 16px rgba(10,22,40,0.2)":"none" }}>
      {children}
    </button>
  );
}
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:24 }}>
      ...
      <div style={{ position:"relative",background:BRAND.white,borderRadius:20,padding:36,maxWidth,width:"100%",boxShadow:"0 24px 80px rgba(10,22,40,0.2)",border:`1px solid ${BRAND.glassBorder}`,maxHeight:"90vh",overflowY:"auto" }}>
        <button onClick={onClose} style={{ position:"absolute",top:16,right:16,background:"none",border:"none",fontSize:20,cursor:"pointer",color:BRAND.textLight }}>&times;</button>
        {children}
      </div>
    </div>
  );
...
function ApplyModal({ campaign, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    await supabase.from("campaign_applications").insert({
      campaign_id: campaign.id, creator_name: form.name, creator_email: form.email,
      pitch: form.pitch, portfolio_url: form.portfolio_url,
      follower_count: parseInt(form.follower_count) || 0, status: "pending",
    });
    ...
    <Modal onClose={onClose}>
      {submitted ? (
        <div style={{ textAlign:"center",padding:"20px 0" }}>
          ...
          <div style={{ fontSize:18,fontWeight:700,color:BRAND.navy,marginBottom:8 }}>Application Sent</div>
          <div style={{ fontSize:13,color:BRAND.textMuted,lineHeight:1.7 }}>The brand will review your profile and reach out if there's a fit.</div>
          <button onClick={onClose} style={{ ...primaryBtn,marginTop:20 }}>Done</button>
        </div>
      ) : (
        <>
          <div style={{ fontSize:10,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",color:BRAND.lightBlue,marginBottom:6 }}>Apply to Campaign</div>
          <h3 style={{ fontSize:18,fontWeight:800,color:BRAND.navy,marginBottom:4 }}>{campaign.title}</h3>
          <div style={{ fontSize:12,color:BRAND.textMuted,marginBottom:20 }}>{campaign.brand_name}{campaign.location ? ` \u00b7 ${campaign.location}` : ""}</div>
          {[["name","Your name *","text"],["email","Your email *","email"],["portfolio_url","Portfolio or social link","text"],["follower_count","Total followers","number"]].map(([k,ph,t])=>(
            <input key={k} type={t} placeholder={ph} value={form[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} style={inp}/>
          ))}
          <textarea placeholder="Why are you the right fit for this campaign? *" value={form.pitch} onChange={e=>setForm(f=>({...f,pitch:e.target.value}))} rows={4} style={{ ...inp,resize:"vertical",minHeight:88 }}/>
...
// FIXED: extracted as proper component — useState cannot be inside .map()
function RFPCard({ c, onApply }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      style={{ background:hovered?BRAND.glassHover:BRAND.glass,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:`1.5px solid ${hovered?BRAND.lightBlue:BRAND.glassBorder}`,borderRadius:16,padding:"22px 20px",transition:"all 0.2s",boxShadow:hovered?BRAND.shadowHover:BRAND.shadow,transform:hovered?"translateY(-3px)":"none" }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10 }}>
        <div>
          <div style={{ fontSize:9,fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",color:BRAND.lightBlue,marginBottom:4 }}>{c.brand_name}</div>
          <div style={{ fontSize:14,fontWeight:800,color:BRAND.navy,lineHeight:1.3 }}>{c.title}</div>
        </div>
        <span style={{ fontSize:9,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",padding:"3px 9px",borderRadius:20,background:BRAND.skyBlue,color:BRAND.midBlue,whiteSpace:"nowrap",flexShrink:0,marginLeft:8 }}>Open RFP</span>
      </div>
      {c.description && <div style={{ fontSize:12,color:BRAND.textMuted,lineHeight:1.65,marginBottom:12 }}>{c.description}</div>}
      <div style={{ display:"flex",gap:14,flexWrap:"wrap",marginBottom:12 }}>
        {c.deliverables>0 && <div style={{ fontSize:11,color:BRAND.textMuted }}><span style={{ fontWeight:700,color:BRAND.navy }}>{c.deliverables}</span> deliverables</div>}
        {c.budget_min>0 && <div style={{ fontSize:11,color:BRAND.textMuted }}><span style={{ fontWeight:700,color:BRAND.green }}>${c.budget_min.toLocaleString()}{c.budget_max>c.budget_min?`\u2013$${c.budget_max.toLocaleString()}`:""}</span></div>}
        {c.location && <div style={{ fontSize:11,color:BRAND.textMuted }}>&#128205; {c.location}</div>}
      </div>
      {(c.platforms||[]).length>0 && (
        <div style={{ display:"flex",gap:4,flexWrap:"wrap",marginBottom:14 }}>
          {c.platforms.map(p=><span key={p} style={{ fontSize:9,fontWeight:700,padding:"3px 8px",borderRadius:6,background:BRAND.navy,color:"#fff" }}>{p}</span>)}
          {(c.niches||[]).map(n=><span key={n} style={{ fontSize:9,fontWeight:600,padding:"3px 8px",borderRadius:6,background:BRAND.skyBlue,color:BRAND.midBlue }}>{n}</span>)}
        </div>
      )}
      <button onClick={()=>onApply(c)} style={{ ...primaryBtn,width:"100%",padding:"11px 0",fontSize:13 }}>Apply Now &rarr;</button>
    </div>
  );
}
 
function RFPList({ onBack }) {
  const [campaigns, setCampaigns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [applying, setApplying] = useState(null);
  const [search, setSearch] = useState("");
  useEffect(() => {
    supabase.from("campaigns").select("*").eq("status","open_rfp").order("created_at",{ascending:false})
  ...
  return (
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,color:BRAND.text }}>
      <header style={{ position:"sticky",top:0,zIndex:100,background:BRAND.glass,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:`1px solid ${BRAND.glassBorder}`,padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10,fontWeight:500,color:BRAND.textMuted,letterSpacing:"2.5px",textTransform:"uppercase" }}>Open RFPs</div>
          </div>
        </div>
        <button onClick={onBack} style={{ ...ghostBtn,padding:"8px 16px",fontSize:12 }}>&larr; Back</button>
      </header>
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"40px 24px 80px" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:12,marginBottom:24 }}>
          <div>
            <h1 style={{ fontSize:28,fontWeight:800,color:BRAND.navy,letterSpacing:"-0.5px" }}>
              {loading ? "Loading..." : `${filtered.length} Open Campaign${filtered.length!==1?"s":""}`}
            </h1>
            <p style={{ fontSize:13,color:BRAND.textMuted,marginTop:4 }}>Brands looking for creators &mdash; apply directly.</p>
          </div>
          <input placeholder="Search campaigns..." value={search} onChange={e=>setSearch(e.target.value)} style={{ ...inp,maxWidth:280,marginBottom:0 }}/>
        </div>
        {loading ? (
          <div style={{ textAlign:"center",padding:60,color:BRAND.textMuted,fontSize:13 }}>Loading opportunities...</div>
        ) : filtered.length === 0 ? (
          <div style={{ background:BRAND.glass,border:`1px solid ${BRAND.glassBorder}`,borderRadius:16,padding:"40px 24px",textAlign:"center" }}>
            <div style={{ fontSize:14,fontWeight:700,color:BRAND.navy,marginBottom:8 }}>No open RFPs right now</div>
            <div style={{ fontSize:12,color:BRAND.textMuted }}>Check back soon.</div>
          </div>
        ) : (
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16 }}>
            {filtered.map(c => <RFPCard key={c.id} c={c} onApply={setApplying} />)}
          </div>
        )}
      </div>
...
function Onboarding({ onComplete, onViewRFPs }) {
  ...
  const toggle = (field, val) => setProfile(p => ({ ...p,[field]:p[field].includes(val)?p[field].filter(x=>x!==val):[...p[field],val] }));
  return (
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:24 }}>
      <div style={{ width:"100%",maxWidth:580,position:"relative",zIndex:1 }}>
        <div style={{ textAlign:"center",marginBottom:40 }}>
          ...
          <p style={{ fontSize:14,color:BRAND.textMuted,maxWidth:420,margin:"0 auto 16px",lineHeight:1.6 }}>Tell us about your content &mdash; we'll find the best Bali businesses to partner with</p>
          <button onClick={onViewRFPs} style={{ fontSize:12,fontWeight:700,color:BRAND.lightBlue,background:BRAND.skyBlue,border:`1px solid ${BRAND.glassBorder}`,borderRadius:20,padding:"6px 18px",cursor:"pointer",fontFamily:"'Montserrat', sans-serif" }}>
            &#128203; Browse Open Brand RFPs &rarr;
          </button>
        </div>
        <div style={{ display:"flex",gap:6,marginBottom:32 }}>
          {[1,2,3].map(s=><div key={s} style={{ flex:1,height:4,borderRadius:2,background:s<=step?BRAND.lightBlue:BRAND.ice,transition:"background 0.3s" }}/>)}
        </div>
        <div style={{ background:BRAND.glass,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",border:`1px solid ${BRAND.glassBorder}`,borderRadius:20,padding:32,boxShadow:BRAND.shadow }}>
          {step===1 && (<>
            ...
            <textarea style={{ ...inp,minHeight:70,resize:"none" }} value={profile.bio} onChange={e=>setProfile({...profile,bio:e.target.value})} placeholder="e.g. Luxury travel creator based in Canggu..."/>
            ...
            {brands.length>0 && (<>
              <label style={lbl}>Link to Pr&euml;mo Dashboard Brand (optional)</label>
          ...
          <div style={{ display:"flex",gap:10,marginTop:24 }}>
            {step>1 && <button onClick={()=>setStep(s=>s-1)} style={ghostBtn}>&larr; Back</button>}
            {step<3
              ? <button onClick={()=>setStep(s=>s+1)} style={{ ...primaryBtn,flex:1 }}>Continue &rarr;</button>
...
function CompassRunning({ profile, onResults }) {
  const phases = ["Scanning Bali business landscape...","Matching by niche and content style...","Scoring fit for your profile...","Generating outreach messages...","Saving your opportunities..."];
  ...
  return (
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:24 }}>
      ...
      <div style={{ textAlign:"center",maxWidth:440 }}>
        ...
        <div style={{ background:BRAND.glass,border:`1px solid ${BRAND.glassBorder}`,borderRadius:16,padding:"20px 24px" }}>
          {phases.map((p,i)=>(
            <div key={p} style={{ display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:i<phases.length-1?`1px solid ${BRAND.skyBlue}`:"none" }}>
              <div style={{ width:20,height:20,borderRadius:"50%",background:i<phase?BRAND.lightBlue:i===phase?BRAND.accentBlue:BRAND.ice,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#fff",flexShrink:0 }}>{i<phase?"✓":""}</div>
              <span style={{ fontSize:13,color:i<=phase?BRAND.navy:BRAND.textLight,fontWeight:i===phase?600:400 }}>{p}</span>
            </div>
          ))}
        </div>
      </div>
...
async function runCompassAgent(profile) {
  const prompt = `You are Creator's Compass. Find 8 perfect Bali business collab partners for this creator. Creator: ${profile.name} | Bio: ${profile.bio} | Platforms: ${profile.platforms.join(", ")} | Followers: ${profile.followers} | Niches: ${profile.niches.join(", ")} | Style: ${profile.style.join(", ")} | Collab types: ${profile.collab.join(", ")} | Location: ${profile.location} Return ONLY a raw JSON array, no markdown: [{"name":"","category":"","location":"","instagram":"@handle","fitScore":95,"fitReason":"one sentence","collabIdea":"specific idea","estimatedValue":"$X - $Y","contactType":"Instagram DM","emoji":"hotel"}] Vary fit scores 70-98. Make businesses specific and realistic for Bali.`;
  try {
    ...
    const {data:creator} = await supabase.from("creators").insert({ name:profile.name,bio:profile.bio,niches:profile.niches,platforms:profile.platforms,followers:profile.followers,style:profile.style,collab_prefs:profile.collab,location:profile.location,brand_id:profile.brand_id||null }).select().single();
    if(creator) {
      await supabase.from("collab_opportunities").insert(businesses.map(b=>({ creator_id:creator.id,business_name:b.name,business_category:b.category,business_location:b.location,business_instagram:b.instagram,fit_score:b.fitScore,fit_reason:b.fitReason,collab_idea:b.collabIdea,estimated_value:b.estimatedValue,contact_type:b.contactType,emoji:b.emoji,status:"new" })));
...
async function generateOutreach(business, profile) {
  const prompt = `Write a short personalized collab DM from ${profile.name||"a creator"} to ${business.name} (${business.category} in ${business.location}). Collab idea: ${business.collabIdea}. Under 120 words. Human and warm. Soft CTA. Return ONLY the message.`;
  try {
  ...
  } catch(e) { return `Hi! I love what you do at ${business.name} and think we could create something amazing together. Would love to chat!`; }
}
 
function getFallback() {
  return [
    {name:"Katamama Boutique Hotel",category:"Luxury Boutique Hotel",location:"Seminyak",instagram:"@katamama",fitScore:96,fitReason:"Perfect for luxury lifestyle content",collabIdea:"3-night stay + infinity pool shoot",estimatedValue:"$1,200 - $3,000",contactType:"Instagram DM",emoji:"🏨"},
    {name:"Deus Ex Machina",category:"Lifestyle Brand & Cafe",location:"Canggu",instagram:"@deuscustoms",fitScore:91,fitReason:"Iconic Bali brand with global reach",collabIdea:"Custom merch feature + cafe morning shoot",estimatedValue:"$500 - $1,500",contactType:"Email",emoji:"🏍️"},
    {name:"Finns Beach Club",category:"Beach Club",location:"Canggu",instagram:"@finnsbeachclub",fitScore:89,fitReason:"Premier beach club for aspirational content",collabIdea:"Pool day + sunset shoot",estimatedValue:"$800 - $2,000",contactType:"Instagram DM",emoji:"🌊"},
    {name:"Avocado Factory",category:"Trendy Cafe",location:"Canggu",instagram:"@avocadofactorybali",fitScore:84,fitReason:"Highly photogenic, massive social following",collabIdea:"Brunch content + story takeover",estimatedValue:"$150 - $400",contactType:"Instagram DM",emoji:"🥑"},
    {name:"Tropicola Beach Club",category:"Beach Club & Restaurant",location:"Seminyak",instagram:"@tropi_cola",fitScore:87,fitReason:"Retro aesthetic, extremely photogenic",collabIdea:"Sunset dinner shoot + reel",estimatedValue:"$400 - $1,000",contactType:"Instagram DM",emoji:"🌅"},
    ...
    {name:"Sea Circus",category:"Restaurant & Bar",location:"Seminyak",instagram:"@seacircusbali",fitScore:82,fitReason:"Vibrant aesthetic, photographs beautifully",collabIdea:"Dinner + cocktail feature",estimatedValue:"$200 - $600",contactType:"Instagram DM",emoji:"🎪"},
...
// FIXED: extracted as proper component — useState cannot be inside .map()
function BizCard({ biz, index, status, onSelect }) {
  const [hovered, setHovered] = useState(false);
  const sc = STATUS_COLORS[status];
  return (
    <div
      style={{ background:hovered?BRAND.glassHover:BRAND.glass,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:`1px solid ${hovered?BRAND.lightBlue:BRAND.glassBorder}`,borderRadius:16,padding:20,cursor:"pointer",transition:"all 0.3s ease",boxShadow:hovered?BRAND.shadowHover:BRAND.shadow,transform:hovered?"translateY(-3px)":"none",display:"flex",flexDirection:"column",gap:10 }}
      onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onClick={()=>onSelect(biz)}>
      <div style={{ width:"100%",height:80,borderRadius:10,background:`linear-gradient(135deg,${BRAND.ice} 0%,${BRAND.skyBlue} 50%,rgba(74,144,217,0.1) 100%)`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative" }}>
        ...
        {index===0&&<span style={{ position:"absolute",top:8,left:8,fontSize:9,fontWeight:700,color:"#fff",background:BRAND.lightBlue,padding:"2px 8px",borderRadius:20 }}>TOP MATCH</span>}
      ...
      <div style={{ fontSize:11,color:BRAND.textLight }}>{biz.category} &middot; {biz.location}</div>
...
function Results({ businesses, profile, onNewSearch, onViewRFPs }) {
    ...
    <div style={{ fontFamily:"'Montserrat', sans-serif",minHeight:"100vh",background:`linear-gradient(165deg,${BRAND.white} 0%,${BRAND.ice} 40%,${BRAND.skyBlue} 100%)`,color:BRAND.text }}>
      <header style={{ position:"sticky",top:0,zIndex:100,background:BRAND.glass,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:`1px solid ${BRAND.glassBorder}`,padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
          ...
          <div>
            <div style={{ fontSize:20,fontWeight:700,color:BRAND.navy,letterSpacing:"1.5px" }}>CREATOR'S COMPASS</div>
            <div style={{ fontSize:10,color:BRAND.textMuted,letterSpacing:"2.5px",textTransform:"uppercase" }}>Collab Opportunities</div>
          </div>
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:12 }}>
          ...
          <button onClick={onViewRFPs} style={{ ...ghostBtn,padding:"8px 16px",fontSize:12 }}>&#128203; Brand RFPs</button>
      ...
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"40px 24px 80px" }}>
        <div style={{ marginBottom:40 }}>
          ...
          <p style={{ fontSize:15,color:BRAND.textMuted,lineHeight:1.6 }}>{profile.niches.join(" \u00b7 ")} &middot; {profile.location}</p>
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16 }}>
          {sorted.map((biz,i) => (
            <BizCard key={biz.name} biz={biz} index={i} status={statuses[biz.name]||"new"} onSelect={setSelected} />
          ))}
        </div>
      </div>
      {selected && (
        <Modal onClose={()=>setSelected(null)}>
          <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:16 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12,color:BRAND.textLight }}>{selected.category} &middot; {selected.location}</div>
            </div>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>
              {STATUSES.map(s=>{ const sc=STATUS_COLORS[s]; const active=(statuses[selected.name]||"new")===s; return <button key={s} onClick={()=>handleStatus(selected,s)} style={{ fontSize:11,fontWeight:700,padding:"4px 10px",borderRadius:20,border:`1.5px solid ${active?sc.text:"transparent"}`,background:active?sc.bg:"transparent",color:active?sc.text:BRAND.textLight,cursor:"pointer",fontFamily:"'Montserrat', sans-serif" }}>{STATUS_LABELS[s]}</button>; })}
            </div>
          </div>
          <div style={{ marginBottom:8 }}>
            <label style={lbl}>Outreach Message</label>
...
export default function App() {
  ...
  return (
    <>
      ...
      {stage==="rfp" && <RFPList onBack={()=>setStage(profile?"results":"onboard")}/>}
      {stage==="onboard" && <Onboarding onComplete={p=>{ setProfile(p); setStage("running"); }} onViewRFPs={()=>setStage("rfp")}/>}
      {stage==="running" && <CompassRunning profile={profile} onResults={r=>{ setResults(r); setStage("results"); }}/>}
      {stage==="results" && <Results businesses={results} profile={profile} onNewSearch={()=>{ setStage("onboard"); setProfile(null); setResults([]); }} onViewRFPs={()=>setStage("rfp")}/>}
    </>
  );
}
